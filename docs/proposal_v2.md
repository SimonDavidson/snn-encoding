...
Closer to home, the Spiketrum encoder is a general-purpose spike-coding algorithm with precisely controllable spike rate and demonstrated signal reconstruction, implemented on both FPGA and ASIC. It is the work of a collaboration led from Zhejiang University by Tang and colleagues, with Wijekoon and Alsakkal among the co-authors at Manchester. It is directly relevant, we have co-authors in the same institution, and it should be in the comparison. Section 5.7 sets out why, and what remains to be settled before it can be included.
...
All candidates share a common first stage, so that differences between them are attributable to the event generation rule rather than to incidental differences in filtering. The stage is a gammatone filterbank, the standard computational approximation to the frequency selectivity of the basilar membrane. E7 is a partial exception in form rather than in substance: it does not apply a filterbank and then an event rule, but decomposes the signal directly over a dictionary of ERB-spaced gammatone atoms, so the filtering is the same in kind and is fused with the event generation rather than preceding it. Section 5.7 returns to this, because it makes E7 a cleaner comparison than an encoder with a genuinely different front end would be. Channel c has impulse response
...
with α~f~ = exp(-Δ t / τ~f~), α~s~ = exp(-Δ t / τ~s~) and τ~s~ > τ~f~. This is a difference of exponentials in time, the temporal analogue of a difference-of-Gaussians in space. Events are emitted by the reference-reset rule of E2 applied to d~c~ rather than to the envelope, separately for the two signs. A reference r~c~ is held on a lattice of spacing θ anchored at zero, and at each sample events are emitted until the reference lies within θ of d~c~:


> r~c~[n] = m~c~[n] θ,  m~c~[0] = 0 ;   while d~c~[n] - r~c~[n] ≥ θ : emit ON, m~c~ → m~c~ + 1 ;   while d~c~[n] - r~c~[n] ≤ -θ : emit OFF, m~c~ → m~c~ - 1   (21)

with m~c~ held as an integer index and the comparisons carrying the tolerance described for E2 in §5.2.

A reset rule rather than a crossing rule, for a reason that is worth stating because the crossing reading is the more natural one to write. Any rule emitting at most one event per crossing has an event count bounded above by the number of excursions of d~c~ through the threshold band, which is a property of the drive and of τ~f~, τ~s~ — not of θ. As θ falls the count saturates rather than growing, so θ cannot act as a rate parameter and the matched-budget comparison of §6.4 cannot be arranged. Measured over an 8× sweep of θ, a crossing rule rearmed at the threshold gives 85, 104, 117, 114, 0 events and one rearmed through zero gives 52, 52, 52, 35, 0; the rule above gives 888, 432, 191, 69, 0. Reading (21) as a level condition with no reset is not an alternative either: it emits on 93 per cent of samples, which contradicts the sparsity claimed two paragraphs below, and the refractory period is a declared constant here rather than a swept axis, so it is not available to rate-limit it.

The consequence to be explicit about is that E3's event count on a transient scales with the transient's amplitude divided by θ, rather than with the number of transients. That is a modelling choice made under pressure from an evaluation requirement, and it is stated here as such. For T3 it is defensible on its own terms — a boundary with greater spectral contrast accumulates proportionally more evidence — but it is a choice, not a consequence of the difference-of-exponentials.

The difference from E2 is worth being precise about, because the two are easily conflated. E2 accumulates: a slow ramp of sufficient total extent will eventually generate events no matter how gradual it is. E3 is bandpass: a ramp slower than τ~s~ produces no response at all, because both filters track it equally and their difference stays near zero. E3 is therefore silent during steady state regardless of level, and responds only to changes fast relative to its slow time constant. This makes it a closer model of the onset-sensitive cells found in the cochlear nucleus, and it produces markedly sparser output on sustained sounds. Note that the two encoders now share an event rule and differ only in equation (20). That is deliberate rather than a concession: it makes the E2-against-E3 comparison a single-factor contrast, so a difference between their Pareto fronts is attributable to the bandpass and to nothing else.
...
Rate parameter: θ = θ~+~ = θ~-~, with the asymmetric case available as a secondary axis. Secondary parameters τ~f~, τ~s~. Δ~ref~ is a declared constant fixed at zero for all comparison runs, never a swept axis, for the reason given for E1 in §5.1: a second rate-limiting mechanism alongside θ would confound the matched-budget comparison of §6.4.
...
Rate parameter: the cycle divisor k. Of the upward zero crossings that survive the envelope gate, every k-th is kept, so the event count falls as 1/k while both the frequency resolution and the per-event timing precision are untouched. This replaces the envelope threshold named in an earlier draft, which cannot serve: the event count is bounded above by the number of upward zero crossings, which the carrier frequency and the drive set and no threshold can move, so the threshold gates rather than modulates. Measured over a 16× sweep it spans 1.04×. The alternative of reaching low budgets by reducing the channel count was rejected because it would remove frequency resolution at the same time, confounding the one thing E5 is in the battery to test. λ~max~ remains the rate parameter of the stochastic form, which is retained but is not a study arm: a Poisson process is shift-stationary rather than shift-equivariant, so it cannot satisfy the time-shift equivariance required of every encoder here. Secondary parameters γ, Δ~ref~, f~lock~, and the envelope cutoff of the internal gate.
...
Rate parameter: the energy fraction e~frac~, with the emission gate at e~frac~ E~max~ and E~max~ the largest frame energy over all channels and frames of the utterance. An absolute E~min~ was specified in an earlier draft and is replaced: any fixed value is meaningful only for one scale of input, and the default chosen sat nearly seven decades below the quietest frame of the test signal, gating nothing. The relative form is scale-free — verified to give identical event counts across five decades of input scale — and it also supplies the E~max~ that equation (28) needs and that no earlier draft defined. The gate is strict, since a non-strict one emits everywhere on silence, and strictness also places every offset strictly inside its frame without a separate clipping rule. The cost is that E6 becomes the only encoder in the battery with utterance-level normalisation, which is noted in §8 as a limitation. Secondarily the hop H.
...
Spiketrum is a general-purpose spike-coding algorithm with published FPGA and ASIC implementations and reported application to auditory perception tasks. It is the work of a collaboration led from Zhejiang University, published as Tang et al., "Neuromorphic Auditory Perception by Neural Spiketrum", IEEE Transactions on Emerging Topics in Computational Intelligence 9(1), 2025; Wijekoon and Alsakkal are among the eight co-authors and are at Manchester. It is a sparse decomposition method in the matching pursuit family, which the authors call Event-based Temporal Matching Pursuit: the signal is approximated by iteratively selecting, from a dictionary of atoms, the atom best correlated with the current residual, emitting an event identifying that atom and its time, and subtracting its contribution. Generically,
...
Three features of the published algorithm bear directly on how E7 would sit in this comparison, and none of them was apparent from the abstracts.

The dictionary is a set of ERB-spaced gammatone atoms, selected for the same auditory-modelling reasons that motivate the front end of Section 5.2. E7 therefore does not differ from the other six candidates in its filtering. It differs in that the filtering and the event generation are one operation rather than two, and in the principle by which events are chosen. That principle is the substantive difference and it is a large one: E1 to E6 all decide whether to emit from local state, in one channel at one moment, whereas matching pursuit selects the single event anywhere in the signal that best explains the remaining residual. E7 is not a seventh point on the continuum the other six occupy; it is the only representative of a second class, and the axis separating it from the others is exactly the axis this study is about.

The amplitude of each selected atom is carried by place rather than by an event payload. The authors' intensity-to-place coding normalises the amplitudes to [0, 1] and expands each of the M dictionary atoms into K units of differing characteristic intensity, routing each code to the unit whose intensity is nearest its coefficient, so that the channel index is K(m − 1) + k [Tang2025, eqs. 3-4]. The characteristic intensities are spaced logarithmically rather than linearly, which the authors motivate by the log-normal distribution of intensity coefficients in natural sound and support by measurement. The output is therefore a unipolar spike train over M × K channels, compatible with the event representation used throughout this study without modification, though its channel index factorises as kernel by quantised intensity rather than as channel by polarity, which is a third convention for the question recorded as O7.

Both M and K are free, and the channel count is their product, so the dimensionality is a configuration choice rather than a fixed property. The only setting the paper states is its FPGA prototype's: 40 gammatone kernels at 3 characteristic intensities, giving 120 output channels per cochlea [Tang2025, §V]. That is inside the range this study already sweeps, so the matched-treatment concern under C4 is real in principle but slight in practice. K = 30 appears in the paper's precision analysis, described there as the small end of the range at which representational precision is retained, and not as a deployed setting.

The rate parameter is the iteration count divided by the signal duration [Tang2025, eq. 2], which is the same quantity as Λ in equation (35) of this proposal. The budget is therefore requested directly rather than approached by adjusting a threshold.

It is not, however, exact. Algorithm 1 terminates on the iteration count *or* on a minimum residual-energy ratio ε_min, whichever comes first, so the achievable rate is capped by a property of the signal once the residual floor binds. That is structurally the same trap found in E5, whose count is capped by the carrier's zero-crossing rate, and in E6, whose count is capped by channels times frame rate. E7's reachable span must therefore be measured against the D27 requirement before it is relied on, not inferred from the form of the rate parameter. The authors report working at rates between roughly 100 Hz and 1.1 kHz for their structure-invariance analysis and at 4 kHz for the information-theoretic one, which overlaps the range our own sweeps occupy, so a matched-budget comparison looks feasible on its face — but that is an argument for measuring rather than a substitute for it.

Two observations. First, its reconstruction capability makes it the direct test of the argument in Section 2.2 rather than merely an extreme point beside it. Spiketrum optimises reconstruction fidelity deliberately and well, which is the criterion Section 2.2 argues is the wrong one for a task-oriented encoder. If a reconstruction-optimal code underperforms local, task-agnostic rules on all three probes at matched budget, that is the strongest result this study could produce, and it is not available without E7. If it does not, Section 2.2 is wrong, which is worth establishing ourselves. That is not a criticism of Spiketrum, which was designed for a different purpose.

It should not, however, be described as an upper reference on a privacy axis, as an earlier version of this section had it. The authors demonstrate reconstruction of the original sound from the spike representation. A representation from which the audio can be recovered carries the same restricted content as the recording; it is not a milder case at one end of a scale but the same case in a different format. Reconstruction fidelity is a claim about information retention and bears on the argument of Section 2.2; it is not a privacy gradient and this proposal should not treat it as one.

Second, it comes with hardware implementations, which is directly relevant if any measured energy claim is later attempted. Third, the published evaluation is on environmental sound events and musical instrument classes, with no speech corpus and no phone, pitch or boundary task. The battery in Section 4 would therefore be new territory for the encoder rather than a repetition of published results.

Status. The description above rests on [Tang2025], read in full. Two earlier versions of this section were wrong, in different ways, and both are worth recording because the section is the validation protocol's worked example of the risk it exists to manage.

The first was reconstructed from published abstracts and citation records. It was substantially right about the algorithm and wrong about its authorship, about E7 not sharing the front end, and about reconstruction constituting a privacy axis.

The second was written after the paper had been read, and introduced errors of its own: a dictionary size of 64 atoms and a consequent channel count of 1920, neither of which appears anywhere in the source; a claim that the rate control is exact, which omits the residual-energy stopping criterion; and no mention of the logarithmic spacing of the characteristic intensities. The first of those is the serious one. It was not an inference from a weaker source but a plausible default supplied for a parameter the paper does not state, written with the same confidence as the material that had actually been read, and it had reached a decision put to a collaborator before the implementation session checked it against the source. Every numeric claim in this section is now tied to a location in [Tang2025]; any that cannot be is removed rather than softened.

A further caution for anyone following the trail. The earlier version named three primary sources — this article, a TCSI article on an FPGA cochlea, and an evaluation paper. That list was itself assembled from citation records. [Tang2025] cites no work by Wijekoon or Alsakkal and describes the FPGA cochlea in its own §V, so whether the other two exist as described is unverified and should not be repeated until one of them is in hand.

What remains open is not technical. The algorithm is fully specified in the published paper and could be implemented without the authors' code, which the paper does not appear to release. Decision D9 nonetheless holds that E7 will not be implemented from published description, on the grounds of not reimplementing a colleague's algorithm badly, and that Oliver and Simon will approach the authors directly. That reasoning is about attribution and collegiality rather than feasibility, and reading the paper does not touch it.

A middle course is now available that was not before, and is put to Oliver as O3: implement the published Event-based Temporal Matching Pursuit algorithm and report it as our implementation of that published algorithm, explicitly not as the authors' Spiketrum system, with the distinction stated wherever results are given. This attributes the algorithm to its authors and the implementation to us, and does not require a meeting that is not currently scheduled. Until O3 is settled, this encoder remains provisional and outside the critical path of Section 9.
...
- **O3 Spiketrum inclusion.** Determines whether E7 enters the comparison and on what terms. Restated 2026-09-11 against [Tang2025]. The technical dependency is gone — the algorithm is fully specified in the published source, which does not appear to release code — and the case for inclusion is stronger than the proposal first assumed: E7 shares the gammatone front end, differs from E1 to E6 in the event-generation rule alone, and is the direct test of the argument in Section 2.2. Two things need Oliver's decision, not three. Whether D9 stands as written, or whether the published algorithm may be implemented and reported as our implementation of it rather than as the authors' system. And whether an approach should go to Wijekoon and Alsakkal as the Manchester co-authors or to Tang as corresponding author at Zhejiang, noting that a request for code or for permission may have to be routed to the latter. The third question previously recorded here — what value of K to use — rested on a channel count that is not in the source; the authors' own prototype answers it with K = 3 at M = 40, which is inside our swept range, so it is a parameter to confirm rather than a decision to take.
