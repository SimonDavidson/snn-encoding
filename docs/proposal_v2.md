...
The amplitude of each selected atom is carried by place rather than by an event payload. The authors' intensity-to-place coding expands each of the M dictionary atoms into K units of differing characteristic intensity, and routes an event to the unit whose intensity is closest to the atom's coefficient. The output is therefore a unipolar spike train over M × K channels, compatible with the event representation used throughout this study without modification. The cost is dimensionality: at the paper's own settings, with K = 30, a 64-atom dictionary yields 1920 channels, against the few dozen this study sweeps. K is a free parameter and the authors report that precision degrades gracefully as it falls, so this is resolvable, but it must be resolved before E7 enters rather than during the sweep, because a feature vector an order of magnitude longer than the other arms' is not obviously matched treatment under C4.

The rate control is exact rather than statistical. The spike rate is the iteration count divided by the signal duration, so the event budget is chosen directly. E7 would be the only arm able to land on a matched budget exactly, where the other six approach one by adjusting a threshold.
...
Status. The description above now rests on the primary source, read in full. An earlier version of this section was reconstructed from abstracts and citation records; it was substantially right about the algorithm and wrong about its authorship, about E7 not sharing the front end, and about reconstruction constituting a privacy axis. Those are corrected above.
...
- **O3 Spiketrum inclusion.** Determines whether E7 enters the comparison and on what terms. Restated 2026-09-09 following a full reading of the primary source, which removes the technical dependency and sharpens the case for inclusion: E7 shares the gammatone front end, differs from the other six only in the event-generation principle, and is the direct test of the argument in Section 2.2. Three things now need Oliver's decision. Whether D9 stands as written, or whether the published algorithm may be implemented and reported as our implementation of it rather than as the authors' system. Whether the approach to the authors should be to Wijekoon and Alsakkal as Manchester co-authors or to Tang as corresponding author. And what value of K the intensity-to-place expansion should use, since the paper's own setting yields a channel count an order of magnitude above the range this study sweeps.
